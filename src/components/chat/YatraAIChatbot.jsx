...
import { FiMessageSquare, FiX, FiSend, FiZap, FiSettings, FiKey, FiCheck, FiRefreshCw } from 'react-icons/fi';
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      sender: 'bot',
      text: 'Namaste! 🇮🇳 I am your YatraAI Assistant powered by Groq Llama-3.3 70B. Ask me about Indian cities, 10 top places to visit, famous regional food, budget hacks, or transit routes!',
      timestamp: 'Just now'
    }
  ]);
  ...
  const handleSend = async (textToSend) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: Date.now() + 1,
        sender: 'bot',
        text: `⚠️ Groq API Note: ${err.message || 'Unable to fetch Groq response.'}\n\nPlease check connection or update Groq Key in settings gear ⚙️.`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }]);
    } finally {
      setIsTyping(false);
    ...
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {!isOpen && (
        <button
          ...
          title="Ask YatraAI Smart Travel Assistant (Groq Cloud Llama-3.3)"
        >
            ...
            <div className="p-4 bg-slate-950 border-b border-slate-800 space-y-3 animate-fade-in">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Leave blank to use default Groq Key"
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3.5 bg-slate-950/40">
            ...
            {isTyping && (
              <div className="flex items-center gap-2 text-slate-400 text-xs font-bold p-2 bg-slate-900/60 rounded-xl w-fit animate-pulse">
                ...
                <span>YatraAI Groq Llama-3 is generating detailed response...</span>
              </div>
            )}
