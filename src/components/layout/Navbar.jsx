...
export default function Navbar() {
  ...
  // Hide global navbar on login page
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <Link to="/home" className="flex items-center gap-2.5 group">
          ...
          <div className="flex flex-col">
            <span className="font-heading font-black text-xl sm:text-2xl tracking-tight text-slate-900 leading-none">
              YatraSathi<span className="text-coral-500">.</span>
            </span>
            <span className="text-[10px] font-bold text-ocean-600 uppercase tracking-wider leading-none mt-1">
        ...
        <nav className="hidden lg:flex items-center gap-6 xl:gap-8">
          {navLinks.map((link) => (
            <NavLink
              ...
              to={link.path}
              className={({ isActive }) =>
                `flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs xl:text-sm font-extrabold transition-all ${
                  isActive
                    ? 'bg-ocean-500 text-white shadow-md shadow-ocean-200'
                    : 'text-slate-700 hover:text-ocean-700 hover:bg-slate-100/80'
                }`
              }
            >
        ...
        <div className="hidden sm:flex items-center gap-3">
          <button
            ...
            className="btn-bounce px-4 py-2 rounded-xl bg-gradient-to-r from-coral-500 to-sunset-500 text-white font-heading font-extrabold text-xs shadow-md shadow-coral-200 flex items-center gap-1.5"
          >
          ...
          {user ? (
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={logout}
                className="text-xs font-bold text-slate-500 hover:text-red-600 px-2 py-1.5 transition-colors"
                title="Logout"
              >
                Logout
              </button>
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white/98 backdrop-blur-xl border-b border-slate-200 px-6 py-6 space-y-3 shadow-2xl animate-fade-in">
          {navLinks.map((link) => (
            <NavLink
              key={link.path}
              to={link.path}
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-extrabold transition-all ${
                  isActive
          ...
          <div className="pt-4 border-t border-slate-100 flex flex-col gap-3">
            <button
              ...
              className="w-full py-3 rounded-2xl bg-gradient-to-r from-coral-500 to-sunset-500 text-white font-heading font-extrabold text-sm shadow-md text-center"
            >
